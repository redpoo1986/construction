...
class XO_Slider_Admin {
	 ...
	 * @since 1.0.0
	 *
	 * @param XO_Slider $parent_object Parent object.
	 */
	 ...
	 * @param void   $default_value Default value.
	 */
	private function get_value( &$params, $name, $default_value = '' ) {
	...
	public function save_post( $post_id ) {
		...
		if ( isset( $_POST['xo_slider_parameters'] ) && is_array( $_POST['xo_slider_parameters'] ) ) {
			...
			$params['thumbs_image_width']   = ( isset( $data['thumbs_image_width'] ) && '' !== trim( $data['thumbs_image_width'] ) ) ? (int) $data['thumbs_image_width'] : '';
			$params['thumbs_image_height']  = ( isset( $data['thumbs_image_height'] ) && '' !== trim( $data['thumbs_image_height'] ) ) ? (int) $data['thumbs_image_height'] : '';
			$params['thumbs_navigation']    = isset( $data['thumbs_navigation'] );
			$params['thumbs_per_view_type'] = isset( $data['thumbs_per_view_type'] ) ? sanitize_text_field( $data['thumbs_per_view_type'] ) : 'auto';
		...
		foreach ( $slides as $key => $value ) {
			...
			if ( 'youtube' === $media_type ) {
				if ( 1 === preg_match( '/[\/?=]([a-zA-Z0-9_\-]{11})[&\?]?/', $media_link, $matches ) ) {
					$image_src = 'https://img.youtube.com/vi/' . $matches[1] . '/default.jpg';
				}
			} elseif ( 'video' === $media_type ) {
				$image_src = get_the_post_thumbnail_url( $media_id, array( 150, 150 ) );
				if ( false === $image_src ) {
		...
		if ( empty( $this->parameters ) ) {
			...
			if ( empty( $this->parameters ) ) {
				$this->parameters = array(
					...
					'thumbs_per_view_type'   => 'auto', // Number of slides per thumbnail view. 'auto' or 'all'.
					'thumbs_per_view'        => '',
	...
	public function display_meta_parameter() {
		...
		$thumbs_image_width     = isset( $params['thumbs_image_width'] ) ? $params['thumbs_image_width'] : '';
		$thumbs_image_height    = isset( $params['thumbs_image_height'] ) ? $params['thumbs_image_height'] : '';
		$thumbs_navigation      = isset( $params['thumbs_navigation'] ) ? $params['thumbs_navigation'] : false;
		$thumbs_per_view_type   = isset( $params['thumbs_per_view_type'] ) ? $params['thumbs_per_view_type'] : 'all';
		...
		echo '<td><input id="xo_slider_parameters[space_between]" name="xo_slider_parameters[space_between]" type="number" value="' . esc_attr( $space_between ) . '" class="small-text" step="1" /> px</td></tr>';
		...
		echo '<tr><th><label for="xo_slider_parameters[thumbs_image_width]">' . esc_html__( 'Image width:', 'xo-liteslider' ) . '</label></th>';
		echo '<td><input id="xo_slider_parameters[thumbs_image_width]" name="xo_slider_parameters[thumbs_image_width]" type="number" value="' . esc_attr( $thumbs_image_width ) . '" class="small-text" min="0" step="1" /> px</td></tr>';
		echo '<tr><th><label for="xo_slider_parameters[thumbs_image_height]">' . esc_html__( 'Image height:', 'xo-liteslider' ) . '</label></th>';
		echo '<td><input id="xo_slider_parameters[thumbs_image_height]" name="xo_slider_parameters[thumbs_image_height]" type="number" value="' . esc_attr( $thumbs_image_height ) . '" class="small-text" min="0" step="1" /> px</td></tr>';
		echo '<tr><th><label for="xo_slider_parameters[thumbs_navigation]">' . esc_html__( 'Navigation:', 'xo-liteslider' ) . '</label></th>';
		echo '<td><input id="xo_slider_parameters[thumbs_navigation]" name="xo_slider_parameters[thumbs_navigation]" type="checkbox" value="1" ' . checked( $thumbs_navigation, true, false ) . '/><label for="xo_slider_parameters[thumbs_navigation]"></label></td></tr>';
		echo '<tr><th><label for="xo_slider_parameters[thumbs_per_view_type]">' . esc_html__( 'View unit:', 'xo-liteslider' ) . '</label></th><td>';
		echo '<select id="xo_slider_parameters[thumbs_per_view_type]" name="xo_slider_parameters[thumbs_per_view_type]">';
		echo '<option value="auto"' . ( 'auto' === $thumbs_per_view_type ? ' selected' : '' ) . '>' . esc_html__( 'Auto', 'xo-liteslider' ) . '</option>';
		echo '<option value="all"' . ( 'all' === $thumbs_per_view_type ? ' selected' : '' ) . '>' . esc_html__( 'All', 'xo-liteslider' ) . '</option>';
		echo '</select>';
		echo '</td></tr>';
		echo '<tr><th></th>';
		...
		echo '<td><input id="xo_slider_parameters[thumbs_space_between]" name="xo_slider_parameters[thumbs_space_between]" type="number" value="' . esc_attr( $thumbs_space_between ) . '" class="small-text" step="1" /> px</td></tr>';
